...
class Token extends PositionComponent with TapCallbacks {
  ...
  void onTapDown(TapDownEvent event) async {
    ...
    // Get the correct token path based on the token's color ID
    String tokenColorId = tokenId.substring(0, 2); // Extract color from tokenId (e.g., 'BT', 'RT')
    String pathId = tokenColorId.replaceFirst('T', 'P'); // Convert 'BT' to 'BP', 'RT' to 'RP', etc.
    List<String> tokenPath = GameState().getTokenPath(pathId);
  ...
  bool spaceToMove() {
    // If token is in base and dice is 6, it can move
    if (state == TokenState.inBase && GameState().diceNumber == 6) {
      return true;
    }
    
    // If token is on board, check if it has space to move
    if (state == TokenState.onBoard) {
      String tokenColorId = tokenId.substring(0, 2);
