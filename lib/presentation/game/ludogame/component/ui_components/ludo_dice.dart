...
class LudoDice extends PositionComponent with TapCallbacks {
  ...
  void onTapDown(TapDownEvent event) async {
    ...
    // Add seed count for game verification
    GameState().addSeedCount(player.playerId, GameState().diceNumber);
    ...
    // Add 1.5 second delay to show dice result
    Future.delayed(const Duration(milliseconds: 1500), () {
      // Show house color selection for player after delay
      _showHouseColorSelection(world, GameState().ludoBoard as LudoBoard, GameState().diceNumber);
    });
  }
  ...
  // Show house color selection for player to choose which house to move
  void _showHouseColorSelection(World world, LudoBoard ludoBoard, int diceNumber) {
    ...
    // Enable all tokens belonging to current player
    for (var token in GameState().currentPlayer.tokens) {
      if (token.state == TokenState.inBase && diceNumber == 6) {
    ...
    final tokensInBase = GameState().currentPlayer.tokens
        .where((token) => token.state == TokenState.inBase)
        .toList();
    final tokensOnBoard = GameState().currentPlayer.tokens
    ...
    // If no tokens can move, switch to next player
    if (!hasMovableTokens) {
