...
class Player {
  ...
  // Helper method to get the tokens that are in home
  List<Token> getTokensInHome() {
    return tokens.where((token) => token.isInHome()).toList();
  }
  
  // Helper method to get the tokens that are in base
  List<Token> getTokensInBase() {
    return tokens.where((token) => token.isInBase()).toList();
  }
  
  // Update total tokens in home count
  void updateTokensInHomeCount() {
    totalTokensInHome = getTokensInHome().length;
  }
